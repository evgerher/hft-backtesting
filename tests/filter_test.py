...
class FilterTest(unittest.TestCase):
  ...
  def test_filter_depth(self):
    ...
    filtered3 = list(filter(lambda x: x[0] == True, map(lambda x: (filter3.filter(x), x), snapshots)))
    filtered4 = list(filter(lambda x: x[0] == True, map(lambda x: (filter4.filter(x), x), snapshots)))
